...
instance : LieRingModule L (M ⊗[A] N) :=
...
instance : LieRinehartModule A L (M ⊗[A] N) where
  lier_smul' x a t := by
    induction t with
    | zero => simp
    | tmul m n =>
...
instance [IsSemilinear A L M] [IsSemilinear A L N] : IsSemilinear A L (M ⊗[A] N) where
  smul_lier_smul a x b t := by
    induction t with
    | zero => simp
    | tmul m n =>
      simp [smul_tmul']
...
instance [IsTrivial A L M] [IsTrivial A L N] : IsTrivial A L (M ⊗[A] N) where
  smul_lier x a t := by
    induction t with
    | zero => simp
    | tmul m n =>
...
@[simp]
private lemma LieRinehartModule.symbol_tmul [IsLinear A L M] [IsLinear A L N] (x : L) (a : A) (m : M) (n : N) :
...
instance [IsLinear A L M] [IsLinear A L N] : IsLinear A L (M ⊗[A] N) where
  symbol_smul x a := by
    ext b t
    induction t with
    | zero => simp
